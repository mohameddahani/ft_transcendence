...
# every gym has different rules, so an answer taken from the wrong gym is visibly wrong
...
# prices come from the catalogue, so the documents always match the database
...
# staff documents hold things a member must never see (discount limits, key safe, late fees)
